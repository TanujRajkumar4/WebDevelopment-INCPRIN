		...
		if (isset($_FILES['file_upl']['name']) and ($_FILES['file_upl']['name'] != "")) {
			...
			if ($check == "pdf" || $check == "doc" || $check == "docx") {
				if (!move_uploaded_file($_FILES['file_upl']['tmp_name'], "../" . $logo)) {
					...
					// echo $qry;
					if ($sql) {
						echo "<script>alert('Inserted Successfully.'); window.location.href = 'file_up.php?p_ty='" . $_GET['p_ty'] . "'&p_id='" . $_GET['p_id'] . "';</script>";
					} else {
						echo "<script>alert('Not Inserted. Retry!'); window.location.href = 'file_up.php?p_ty='" . $_GET['p_ty'] . "'&p_id='" . $_GET['p_id'] . "';</script>";
					}
				}
			} else {
	...
	if ($_GET['p_ty'] == "tech") {
		...
		if (unlink("../" . $logo)) {
			...
			if ($sql) {
				...
				echo "<script>alert('Not Deleted. Retry!'); window.location.href = 'file_up.php?p_ty='" . $_GET['p_ty'] . "'&p_id='" . $_GET['p_id'] . "';</script>";
			}
		}
	} else {
						...
						<div class="card-footer clearfix">
							<ul class="pagination pagination-sm m-0 float-right">
								...
								for ($page = 1; $page <= $num_pg; $page++) {
									echo '<li class="page-item"><a class="page-link" href = "file_up.php?p_id=' . $_GET['p_id'] . '&p_ty=' . $_GET['p_ty'] . '&page=' . $page . '">' . $page . ' </a>';
								}
								?>
