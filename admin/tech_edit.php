        ...
        if (isset($_FILES['file_upl']['name']) and ($_FILES['file_upl']['name'] != "")) {
            ...
            if ($check == "png" || $check == "jpeg" || $check == "jpg" || $check == "png") {
                if (!move_uploaded_file($_FILES['file_upl']['tmp_name'], "../" . $logo)) {
                    ...
                    echo "<script>window.location.href ='tech_edit.php?id=" . $s_id . "';</script>";
                } else {
                    ...
                    if (@unlink("../" . $logo_old)) {
                        ...
                        // echo $qry;
                        if ($sql) {
                            echo "<script>alert('Edit Successful.'); window.location.href = 'tech.php';</script>";
                        } else {
                            echo "<script>alert('Not Edited. Retry!'); window.location.href = 'tech_edit.php?id=" . $s_id . "';</script>";
                        }
                    } else {
                        $qry = "UPDATE `inc_service` SET `service_name`='$msg',`service_img`='$logo',`service_desc`='$desc' WHERE service_id = '$s_id'";
                        $sql = mysqli_query($dbConn, $qry);
                        // echo $qry;
                        if ($sql) {
                            echo "<script>alert('Edit Successful. Error Deleting OLD Video/Image. Delete Manually from the Server'); window.location.href = 'tech.php';</script>";
                        } else {
                            echo "<script>alert('Not Edited. Retry!'); window.location.href = 'tech_edit.php?id=" . $s_id . "';</script>";
                        }
                    }
                    ...
                    <div class="card card-primary">
                        ...
                        <form method="POST" action="" enctype="multipart/form-data">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    <label for="serviceImage">Current Technology Image</label><br>
                            ...
                            <div class="card-footer">
                                ...
                                <input type="button" class="btn btn-success" name="clr_features" Value="Back" onclick="window.location.assign('tech.php#SL')">
                            </div>
                        </form>
                    </div>
                </div>
